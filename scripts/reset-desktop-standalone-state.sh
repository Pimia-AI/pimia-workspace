...
if [[ "$instance_id" != "es.pimia.workspace.dev" && "$instance_id" != es.pimia.workspace.dev.* ]]; then
    echo "reset-desktop-standalone-state: refusing non-dev bundle identifier: $instance_id" >&2
    exit 1
fi
if [[ "$keyring_service" != "pimia-workspace-desktop-dev" && "$keyring_service" != pimia-workspace-desktop-dev.* ]]; then
