...
/// ([`crate::app_state::keyring_config::MAIN_DEV_KEYRING_SERVICE`]). Only runs in debug builds —
...
/// somebody else's Nostr identity.
///
...
pub fn migrate_agent_keys_to_dev_service(app: &tauri::AppHandle) {
    ...
    if !cfg!(feature = "system-keyring")
        || keyring_service() != crate::app_state::keyring_config::MAIN_DEV_KEYRING_SERVICE
    {
        return;
    }
    ...
    // singleton's cache.
    let prod_store = crate::secret_store::SecretStore::keyring("pimia-workspace-desktop");
