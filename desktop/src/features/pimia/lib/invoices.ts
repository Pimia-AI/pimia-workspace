 ...
 *
 * ✅ La aritmética civil (`parseCivilDate` / `daysBetween`) ya **no** está aquí
 * abajo: vive en `lib/civilDates.ts`, que es lo que este mismo docblock pedía
 * por su nombre. Lo que la sacó fue el segundo cliente —el preaviso de
 * caducidad de los presupuestos, `lib/estimates.ts`—, y la razón sigue siendo
 * la de entonces: una regla escrita dos veces es una regla que un día se
 * «simplifica» en uno de los dos sitios.
 */
